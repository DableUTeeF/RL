...
try:
    ...
    raise error.DependencyNotInstalled(
        '{}. (HINT: you may need to install the Go dependencies via "pip install gym[pachi]".)'.format(e))
...
class GoEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.reset_opponent(self.state.board)

        # Let the opponent play if it's not the agent's turn
        opponent_resigned = False
        if self.state.color != self.player_color:
            self.state, opponent_resigned = self.exec_opponent_play(self.state, None, None)
    ...
    def step(self, action):
        ...
        if not self.state.board.is_terminal:
            self.state, opponent_resigned = self.exec_opponent_play(self.state, prev_state, action)
